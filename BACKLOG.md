...
- small · Write the `specs/`-not-`docs/` layout constraint into `CLAUDE.md`. — replaced the parked build-tier-scaffold row (amended 2026-08-23); the scaffold is still not owed until a spec here is declared build tier
- one-liner · ~~`docs/events.json` caches aggressively~~ — fixed in v0.5, done 2026-08-15
- one-liner · Two rough edges in the v0.5 `¶` overlay — both seen in the shipped page
- medium · The weekly run is automated — and publishes unreviewed — **active**; the owner's two-phase check is outstanding

---
- medium · Automated fact-check pass before an unattended publish
